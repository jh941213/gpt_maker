...
st.write("기존 ChatGPT의 응답의 input token의 제한과 , GPT-4turbo 를 API 에서만 호출이 가능한 이점을 개선")
st.write("프롬프트를 최적화 및 반복 사용으로 효율성을 높이기 위해 제작")
...
with col1:
    if st.button("SEO 최적화 블로그 글 작성하기"):
        try:
            file_path = "gptmaker/prompts/0_prompt.txt"
...
with col2:
    if st.button("문어체로 작성하기"):
        try:
            file_path = "gptmaker/prompts/1_prompt.txt"
            prompt = read_prompt(file_path)
            response = generate_response(client, model_choice, prompt, user_input)
